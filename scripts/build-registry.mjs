...
const META_FILE = 'projects/demo/src/app/generated/meta.ts';

/**
 * The released version, which lives in a git tag rather than in any file: the root package is
 * private at 0.0.0 and semantic-release stamps `dist/` at publish time, never the source. Falls
 * back to whatever was generated last, so a shallow clone with no tags cannot blank it out.
 */
function releasedVersion() {
  try {
    ...
    const previous = existsSync(join(root, META_FILE))
      ? / version: '([^']*)'/.exec(read(META_FILE))
      : null;
...
// The numbers the site puts on screen, so a release never leaves a stale badge behind.
writeFileSync(
  join(root, META_FILE),
