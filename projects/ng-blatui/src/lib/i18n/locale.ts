...
export interface BuiWeekInfo {
   ...
   * of Europe uses, `1` in the US, where the week containing Jan 1 is always week 1.
   */
  minimalDays: number;
}
...
interface LocaleWeekInfo {
  getWeekInfo?: () => { firstDay: number; weekend: readonly number[]; minimalDays: number };
  weekInfo?: { firstDay: number; weekend: readonly number[]; minimalDays: number };
}
