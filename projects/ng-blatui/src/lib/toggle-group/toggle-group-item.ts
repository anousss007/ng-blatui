...
export class BuiToggleGroupItem {
  ...
  protected readonly computedClass = computed(() =>
    cn(
      'inline-flex h-9 min-w-9 items-center justify-center gap-2 border border-input px-2.5 text-sm font-medium transition-colors outline-none not-first:border-s-0 first:rounded-s-md last:rounded-e-md hover:bg-muted focus-visible:z-10 focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 data-[state=on]:bg-accent data-[state=on]:text-accent-foreground',
      this.userClass(),
    ),
  );
}
